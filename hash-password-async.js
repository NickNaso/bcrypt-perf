...
async function run() {
    try {
        ...
        for (let i = 0; i < REPETITION; i++) {
            ...
            await bnapi.hash(PASSWORD, await bnapi.genSaltSync(SALT_ROUNDS))
        ...
        await csvExport('napi-hash-password-async.csv', napiRes, opts)  
        let nanRes = []
        for (let i = 0; i < REPETITION; i++) {
            let time = process.hrtime()
            await bnan.hash(PASSWORD, await bnan.genSalt(SALT_ROUNDS))
            let diff = process.hrtime(time)
            let total = diff[0] * NS_PER_SEC + diff[1]
            nanRes.push({
                ID: i + 1,
        ...
        await csvExport('nan-hash-password-async.csv', nanRes, opts)
        process.exit(0)
    } catch (err) {
