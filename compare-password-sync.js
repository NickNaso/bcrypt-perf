...
async function run() {
    try {
        ...
        for (let i = 0; i < REPETITION; i++) {
            ...
            bnapi.compareSync(PASSWORD, HASHED_PASSWORD)
        ...
        await csvExport('napi-compare-password-sync.csv', napiRes, opts)  
        let nanRes = []
        for (let i = 0; i < REPETITION; i++) {
            let time = process.hrtime()
            bnan.compareSync(PASSWORD, HASHED_PASSWORD)
            let diff = process.hrtime(time)
            let total = diff[0] * NS_PER_SEC + diff[1]
            nanRes.push({
                ID: i + 1,
        ...
        await csvExport('nan-compare-password-sync.csv', nanRes, opts)
        process.exit(0)
    } catch (err) {
